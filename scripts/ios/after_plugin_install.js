 ...
 * @brief Hook script that runs after the functions plugin is installed on iOS.
 *
 * Updates the FirebaseFunctions pod version in the Podfile based on the
...
var PLUGIN_ID = "cordova-plugin-firebasex-functions";
...
function resolvePluginVariables(context) {
    ...
    try {
        ...
        if (fs.existsSync(configXmlPath)) {
            ...
            console.warn("[FirebasexFunctions] config.xml not found at " + configXmlPath + ". Cannot read plugin variables from config.xml.");
        }
    } catch (e) {
        console.warn("[FirebasexFunctions] Could not read config.xml for plugin variables: " + e.message);
    }
    ...
    try {
        ...
        if (fs.existsSync(packageJsonPath)) {
            ...
            console.warn("[FirebasexFunctions] package.json not found at " + packageJsonPath + ". Cannot read plugin variables from package.json.");
        }
    } catch (e) {
        console.warn("[FirebasexFunctions] Could not read package.json for plugin variables: " + e.message);
    }
 ...
 * Updates the FirebaseFunctions pod version in the Podfile.
 *
 * @param {object} context - The Cordova hook context.
 */
module.exports = function(context) {
    ...
    if (!pluginVariables["IOS_FIREBASE_SDK_VERSION"]){
        console.warn("[FirebasexFunctions] IOS_FIREBASE_SDK_VERSION variable not set. Skipping Podfile update for FirebaseFunctions pod version.");
        return;
    }
    ...
    if (!fs.existsSync(podFilePath)) {
        console.warn("[FirebasexFunctions] Podfile not found at " + podFilePath + ". Cannot update FirebaseFunctions pod version.");
        return;
    }
    ...
    try {
        ...
        var podRegEx = /pod 'FirebaseFunctions', '(\d+\.\d+\.\d+[^'"]*)'/g;
        ...
        if (matches) {
            ...
            if (modified) {
                ...
                console.log("[FirebasexFunctions] Firebase SDK version set to v" + sdkVersion + " in Podfile");
            }
        }
    } catch(e) {
        console.warn("[FirebasexFunctions] Error updating Firebase Functions pod version: " + e.message);
    }
};
